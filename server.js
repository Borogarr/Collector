...
function isMultiPart(req,res,next) {
...
app.post('/item/upload', isMultiPart, function(req, res, next){
	  // the uploaded file can be found as `req.files.image` and the
	  // title field as `req.body.title`
	  mkdirp(__dirname + "/app/images/" + req.session.userId, function(err) {
			if (err) {
			}
		});
	  fs.readFile(req.files.file.path, function(err, data) {
		var newPath = __dirname + "/app/images/"+req.session.userId+"/"+ req.files.file.name;
		fs.writeFile(newPath, data, function(err) {
			if (err) throw err;
		});
		
	});
	  res.send(format("/images/" + req.session.userId+"/"+ req.files.file.name))
	});
...
app.namespace('/api', function() {
	...
	app.get('/items/:itemId', restrict, function(request,response) {
		Item.findById(request.params.itemId).exec(function(error, item) {
			if (error) {
				response.send(error);
			}
			response.json(item);
	...
	//single
	app.get('/collection/:collectionId', restrict, function(request, response) {
		Collection.findById(request.params.collectionId).exec(function(error, collection) {
			if (error) {
				response.send(error);
			}
			response.json(collection);
	...
	app.get('/users/', restrict, function(request,response) {
		User.findById(request.session.userId).exec(function(error, userId) {
			if (error) {
				response.send(error);
			}
			response.json(userId);
